            ...
            return (
              <div key={item.label} className="group flex h-full min-w-[22px] flex-1 flex-col items-center justify-end gap-1">
                <div className="relative flex h-full w-full items-end justify-center gap-0.5">
                  <div className="w-2.5 rounded-t bg-[#20F4FF]/80 transition-all group-hover:bg-[#20F4FF]" style={{ height: item.count ? `${Math.max((item.count / maximum) * 100, 3)}%` : '0%' }} title={`${item.label}: ${item.count} downloads`} />
                  <div className="w-2.5 rounded-t bg-[#FF2D95]/80 transition-all group-hover:bg-[#FF2D95]" style={{ height: uniqueCount ? `${Math.max((uniqueCount / maximum) * 100, 3)}%` : '0%' }} title={`${item.label}: ${uniqueCount} unique`} />
                </div>
